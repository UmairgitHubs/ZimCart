...
const VoucherCard = ({ item }: { item: Voucher }) => {
  ...
  const copyToClipboard = async () => {
    try {
      ...
      Alert.alert('Copied', `Code ${item.code} copied to clipboard.`);
    } catch {
        ...
        <View className="flex-1 p-4 justify-between bg-gray-50">
          ...
          {isActive && (
            <View className="flex-row items-center justify-between mt-3 pt-3 border-t border-dashed border-gray-200">
              ...
              <Pressable onPress={copyToClipboard} className="flex-row items-center active:opacity-60">
...
export default function VouchersScreen(props: { navigation?: { goBack: () => void } }) {
  ...
  const validateMutation = useMutation({
    mutationFn: (code: string) => customerApi.validateVoucher(code),
    onSuccess: (v) => {
      const off =
        v.discountType === 'PERCENTAGE' ? `${v.value}% off` : `Rs. ${v.value} off`;
      const min = v.minSpend != null ? ` Minimum spend Rs. ${v.minSpend}.` : '';
      Alert.alert('Valid code', `${off}.${min} Use it at checkout in your cart.`);
      setApplyCode('');
    },
    onError: (e) => {
      Alert.alert('Invalid code', parseApiError(e));
    },
  });

  const displayedVouchers = useMemo(() => {
    const v = vouchers ?? [];
    if (filter === 'active') return v.filter((x) => x.status === 'active');
    return v;
  }, [vouchers, filter]);
  ...
  const handleApply = () => {
    ...
    if (!code) {
      Alert.alert('Enter a code', 'Type your voucher code first.');
      return;
    }
      ...
      >
        {isLoading ? (
          <View className="items-center justify-center py-16">
            ...
            <Text className="text-gray-500 mt-3 font-medium">Loading vouchers…</Text>
          </View>
        ) : (
          <>
            ...
            {displayedVouchers.length === 0 && (
              <View className="items-center justify-center mt-12">
                <Text className="text-gray-500">No vouchers in this list</Text>
              </View>
            )}
          </>
        )}
        ...
        <View className="mt-8 mb-4">
          <Text className="text-gray-900 font-bold mb-3">Check a voucher code</Text>
          <View className="flex-row">
            <TextInput
              className="flex-1 bg-white border border-gray-200 rounded-xl px-4 py-3 mr-3 text-gray-900"
          ...
          <Text className="text-xs text-gray-400 mt-2 leading-5">
            Confirms the code with the server. Adding a new code to your wallet may require admin assignment or a
            future “claim” API.
          </Text>
        </View>
      </ScrollView>
