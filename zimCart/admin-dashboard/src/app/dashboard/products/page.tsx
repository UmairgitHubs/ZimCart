...
import { MOCK_PRODUCTS, PRODUCT_CATEGORIES, PRODUCT_STATUSES } from "@/constants/products";
...
export default function ProductsPage() {
  ...
  const handleEdit = (product: Product) => console.log("Edit:", product.id);
  const handleDelete = (product: Product) => console.log("Delete:", product.id);
  const handleView = (product: Product) => console.log("View:", product.id);
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <section className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <StatCard 
          label="Low Stock" 
          value={MOCK_PRODUCTS.filter(p => p.status === 'Low Stock').length} 
          icon={AlertTriangle} 
        ...
        <StatCard 
          label="In Store" 
          value={MOCK_PRODUCTS.filter(p => p.status === 'In Stock').length} 
          icon={CheckCircle2} 
          color="text-blue-600" 
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="px-8 py-6 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between bg-slate-50/30 gap-4">
                <p className="text-xs font-black text-slate-400">
                  Showing <span className="text-slate-700">{filteredProducts.length}</span> of {MOCK_PRODUCTS.length} products
                </p>
