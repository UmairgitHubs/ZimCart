...
import { AlertTriangle, Trash2, X, ShieldOff } from "lucide-react";
...
export function DeleteRiderModal({ isOpen, onClose, onConfirm, rider }: DeleteRiderModalProps) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-md bg-white rounded-[32px] shadow-2xl p-8 animate-in zoom-in-95 duration-300">
        ...
        <p className="text-slate-500 mt-4 text-sm font-medium leading-relaxed">
          Are you sure you want to remove <span className="font-extrabold text-slate-800">"{rider.name}"</span> from the active fleet? 
          This will immediately revoke their access to the Rider App and clear their current queue. This action <span className="text-red-600 font-bold underline decoration-red-200 underline-offset-4">cannot be reversed</span>.
        </p>
        ...
        <div className="bg-red-50/50 border border-red-100 p-4 rounded-2xl mt-6 flex items-start gap-3">
           ...
           <p className="text-[11px] font-bold text-red-700 leading-normal">
             Warning: Removing a rider with active deliveries may cause fulfillment delays. Ensure all pending orders are reassigned before termination.
           </p>
        </div>
