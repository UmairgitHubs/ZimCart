...
async function getManagedStoreId(managerId) {
...
async function resolveStoreScope(user, queryStoreId) {
...
async function assertAdminStoreExists(storeId) {
...
function sqlOrderStore(storeId) {
    if (storeId === undefined || storeId === null)
        return Prisma.sql ``;
    return Prisma.sql `AND o."storeId" = ${storeId}`;
}
function normalizeRange(raw) {
    if (raw === '7d' || raw === '30d' || raw === 'quarter' || raw === 'ytd')
        return raw;
    return 'ytd';
}
...
async function orderKpis(start, end, storeId) {
...
async function categoryBarsLast30(storeId) {
    if (storeId === undefined)
        return [];
    const end = endOfToday();
...
async function statusCounts(start, end, storeId) {
    if (storeId === undefined)
        return {};
    const rows = await prisma.$queryRaw `
...
async function revenueTimeline(range, storeId) {
    if (storeId === undefined)
        return [];
...
async function categoryPerformance(range, storeId) {
    if (storeId === undefined)
        return [];
    const { start, end } = windowForRange(range);
    const rows = await prisma.$queryRaw `
...
async function topProducts(range, storeId) {
    if (storeId === undefined)
        return [];
    const { start, end } = windowForRange(range);
    const rows = await prisma.$queryRaw `
...
async function regionBreakdown(range, storeId) {
    if (storeId === undefined)
        return [];
    const { start, end } = windowForRange(range);
    const rows = await prisma.$queryRaw `
...
export async function getStaffOverview(user, queryStoreId) {
    ...
    const prevAov = prev.activeOrders > 0 ? Math.round((prev.revenue / prev.activeOrders) * 100) / 100 : 0;
    const fulfillment = cur.activeOrders > 0 ? Math.round((cur.completed / cur.activeOrders) * 1000) / 10 : 0;
    const prevFulfillment = prev.activeOrders > 0 ? Math.round((prev.completed / prev.activeOrders) * 1000) / 10 : 0;
    return {
...
export async function getStaffInsights(user, opts) {
