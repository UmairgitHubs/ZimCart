...
const getEffectiveStoreId = async (providedId, managerId) => {
    if (providedId)
        return providedId;
    ...
    const store = await prisma.store.findFirst();
    if (!store)
        throw new ApiError(400, "Store context not found. Please create a store first.");
    return store.id;
};
export const createProduct = async (data, managerId) => {
    try {
...
export const updateProduct = async (id, data, managerId) => {
    try {
        const storeId = await getEffectiveStoreId(undefined, managerId);
        let targetCategoryId;
        ...
        const updateData = Object.fromEntries(Object.entries(rest).filter(([_, v]) => v !== undefined));
        if (targetCategoryId) {
...
export const getProductById = async (id, options) => {
    ...
    if (options?.marketplaceMode) {
        ...
        if (!s || !s.isActive)
            return null;
        if (s.status !== 'OPEN' && s.status !== 'BUSY')
            return null;
    }
    return product;
};
export const deleteProduct = async (id, managerId) => {
    try {
        const storeId = await getEffectiveStoreId(undefined, managerId);
