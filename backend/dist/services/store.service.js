...
export async function resolveManagedStoreId(userId) {
...
export async function getStoreSettingsForStaff(staff, queryStoreId) {
    let storeId = null;
    if (staff.role === 'STORE_MANAGER') {
        storeId = await resolveManagedStoreId(staff.id);
        if (!storeId)
            throw new ApiError(403, 'No managed store linked to this account');
    }
...
export async function updateStoreSettingsForStaff(staff, input) {
    let targetId = null;
    if (staff.role === 'STORE_MANAGER') {
        ...
        if (!input.storeId)
            throw new ApiError(400, 'storeId is required in body for admin');
    ...
    if (patch.deliveryTime !== undefined)
        data.deliveryTime = patch.deliveryTime;
    if (patch.minOrder !== undefined)
        data.minOrder = patch.minOrder;
    if (patch.deliveryFee !== undefined)
        data.deliveryFee = patch.deliveryFee;
...
const publicMartDiscoveryWhere = {
    isActive: true,
    status: { in: ['OPEN', 'BUSY'] },
};
...
export const getMartById = async (id, search, category) => {
    const storeBaseWhere = {
        id,
