...
export const loginWithTokens = async (user, deviceInfo) => {
    ...
    notificationService.sendNotification([user.id], 'New Login Detected', `A new login was successful on your ZimCart account from ${deviceInfo?.os || 'a new device'} (${deviceInfo?.ipAddress || 'unknown IP'}). If this wasn't you, please change your password immediately.`, { type: 'SECURITY' }).catch(console.error);
    const { password: _, refreshToken: __, twoFactorCode: ___, twoFactorExpires: ____, ...userResponse } = user;
    return { user: userResponse, accessToken, refreshToken };
};
export const register = async (data, deviceInfo) => {
...
export const login = async (data, deviceInfo) => {
    ...
    if (user.isTwoFactorEnabled) {
        ...
        return {
            mfaRequired: true,
            mfaToken,
            email: user.email.replace(/(.{2})(.*)(?=@)/, (_gp1, gp2, gp3) => gp2 + "*".repeat(gp3.length))
        };
    }
    return await loginWithTokens(user, deviceInfo || {});
};
export const verify2FA = async (mfaToken, code, deviceInfo) => {
    try {
        ...
        const accessToken = generateAccessToken({ ...user, sessionId: session.id });
        const refreshToken = generateRefreshToken({ id: user.id, sessionId: session.id });
        notificationService.sendNotification([user.id], 'New Login Detected', `A new login was successful on your ZimCart account via 2FA from ${deviceInfo?.os || 'a new device'}.`, { type: 'SECURITY' }).catch(console.error);
        const { password: _, refreshToken: __, twoFactorCode: ___, twoFactorExpires: ____, ...userResponse } = user;
        return { user: userResponse, accessToken, refreshToken };
    }
    catch (error) {
...
export const resend2FA = async (mfaToken) => {
    try {
        const decoded = verifyAccessToken(mfaToken);
        if (decoded.type !== 'mfa')
            throw new ApiError(401, "Invalid MFA token");
