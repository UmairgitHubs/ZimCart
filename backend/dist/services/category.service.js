...
export const getCategories = async (params) => {
    ...
    if (user?.role === 'STORE_MANAGER') {
        ...
        if (!store)
            return { items: [], stats: { total: 0, published: 0, draft: 0, hidden: 0 } };
    ...
    const statsWhere = { storeId: where.storeId };
    if (where.OR)
...
export const createCategory = async (data, user) => {
    if (user.role === 'STORE_MANAGER') {
        const store = await prisma.store.findFirst({ where: { managerId: user.id } });
        if (!store)
            throw new ApiError(403, "Store context not found. Cannot create category.");
