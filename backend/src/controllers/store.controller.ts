...
export const getStoreSettings = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, { store: serializeSettingsStore(store) }, 'Store settings fetched successfully')
  );
});
...
export const updateStoreSettings = asyncHandler(async (req, res) => {
  ...
  const updated = await storeService.updateStoreSettingsForStaff(req.user!, {
    ...
    deliveryTime: body.deliveryTime === undefined ? undefined : (body.deliveryTime as string | null),
    ...
    status:
      typeof statusVal === 'string' && ['OPEN', 'CLOSED', 'BUSY', 'HIDDEN'].includes(statusVal)
        ? (statusVal as StoreStatus)
        : undefined,
    openingHours: body.openingHours as Prisma.InputJsonValue | undefined,
  });

  return res.status(200).json(
    new ApiResponse(200, { store: serializeSettingsStore(updated) }, 'Store settings updated successfully')
  );
});
