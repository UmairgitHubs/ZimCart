...
import type { Prisma, RiderAvailability, UserStatus } from '@prisma/client';
import prisma from '../config/db.js';
import { ApiError } from '../utils/ApiError.js';

export type RiderUiStatus = 'Available' | 'Dispatched' | 'Offline' | 'Banned';

function toUiStatus(user: { status: string }, availability: RiderAvailability | null): RiderUiStatus {
  if (user.status === 'BLOCKED') return 'Banned';
...
function uiStatusToDb(ui: RiderUiStatus): { userStatus: UserStatus; availability: RiderAvailability } {
  if (ui === 'Banned') return { userStatus: 'BLOCKED', availability: 'OFFLINE' };
  if (ui === 'Offline') return { userStatus: 'ACTIVE', availability: 'OFFLINE' };
  if (ui === 'Dispatched') return { userStatus: 'ACTIVE', availability: 'DISPATCHED' };
  return { userStatus: 'ACTIVE', availability: 'AVAILABLE' };
}

function formatLastActive(sessions: { lastActive: Date }[]): string {
...
const riderInclude = {
  riderProfile: true,
  sessions: {
    orderBy: { lastActive: 'desc' as const },
    take: 1,
...
export async function listRiders(params: {
  ...
  for (const u of allForStats) {
    ...
    if (s === 'Available') availableNow++;
    else if (s === 'Dispatched') onDelivery++;
    else if (s === 'Banned') banned++;
    else offline++;
  }

  return {
    riders: rows.map((row) => serializeRider(row as never)),
...
export async function createRider(input: {
  ...
  return serializeRider(user as never);
}
...
export async function updateRider(
  ...
  if (!user) throw new ApiError(404, 'Rider not found');
  ...
  const profilePatch: Prisma.RiderProfileUpdateInput = {};
  if (input.nationalId !== undefined) profilePatch.nationalId = input.nationalId;
  if (input.vehicleType !== undefined) profilePatch.vehicleType = input.vehicleType;
  if (input.licensePlate !== undefined) profilePatch.licensePlate = input.licensePlate;
  if (input.homeBaseLabel !== undefined) profilePatch.homeBaseLabel = input.homeBaseLabel;
  if (input.completedDropoffs !== undefined) profilePatch.completedDropoffs = input.completedDropoffs;
  if (input.rating !== undefined) profilePatch.rating = input.rating;
  ...
  if (input.status !== undefined) {
    ...
    if (input.accountStatus !== undefined) userPatch.status = input.accountStatus;
    if (input.availability !== undefined) profilePatch.availability = input.availability;
  }

  const data: Prisma.UserUpdateInput = { ...userPatch };
    ...
    if (user.riderProfile) {
      ...
      data.riderProfile = {
        create: {
          ...
          licensePlate: input.licensePlate ?? null,
          homeBaseLabel: input.homeBaseLabel ?? null,
          availability: (profilePatch.availability as RiderAvailability | undefined) ?? 'OFFLINE',
  ...
  return serializeRider(updated as never);
}
