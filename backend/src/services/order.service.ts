...
export const getAllOrders = async (query: any, user: any) => {
    ...
    if (user.role === 'STORE_MANAGER') {
        ...
        if (!storeId) return []; // No store, no orders
    ...
    const mappedOrders = orders.map(order => {
        ...
        switch(order.status) {
            ...
            case "CONFIRMED": mappedStatus = "Confirmed"; break;
            case "PREPARING": mappedStatus = "Confirmed"; break; // Collapse preparing
            case "SHIPPING": mappedStatus = "Shipped"; break;
        ...
        return {
            ...
            items: order.items.map(item => {
                const snappedName = (item as any).name;
...
export const updateOrderStatus = async (orderId: string, newStatus: string, user: any) => {
    ...
    const where: any = { orderNumber: orderId };
